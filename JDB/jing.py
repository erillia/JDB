...
class typer:
    def splitter(lst , chunk_count):
        length = len(lst)
        ap = length % chunk_count
        objs = (length-ap)/chunk_count+1
        start = 0
        objs = round(objs)
        end = objs
        for i in range(chunk_count):
            yield lst[start:end]
            start , end = end , end+objs
...
class connect:
    def __init__(self , db , compressed = False,loop_save = None):
...
"""     
        if loop_save:
            ...
            scheduler.add_job(self.commit, "interval", seconds=loop_save)
            scheduler.start()
        ...
        self.thread_count = psutil.cpu_count() // psutil.cpu_count(logical=False)
...
param conditions  = finder object
    ...
    def FastGiveTable(self , name , finder):
        obj , cond , value = finder['obj'] , finder['cond'] , finder['value']
        table = self.data['__table__'][name]
